...
func TestApplyRolloutStatusImmediate(t *testing.T) {
	...
	result := core.RolloutResult{Planned: []string{"a", "b"}, Synced: []string{"a", "b"}, TotalTargets: 2, CompletedCount: 2}
...
func TestApplyRolloutStatusRolling(t *testing.T) {
	cp := &ConfigPropagation{}
	result := core.RolloutResult{Planned: []string{"batch"}, Synced: []string{"batch"}, TotalTargets: 5, CompletedCount: 2}
	cp.ApplyRolloutStatus(result)
	if cp.Status.TargetCount != 5 || cp.Status.SyncedCount != 1 || cp.Status.OutOfSyncCount != 3 {
