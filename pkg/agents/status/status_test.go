...
func TestMergeConditionsDeterministicOrder(t *testing.T) {
	previous := []core.Condition{
		{Type: core.CondProgressing, Status: "False", Reason: "Idle", Message: "-", LastTransitionTime: "old"},
		{Type: core.CondReady, Status: "True", Reason: "Reconciled", Message: "ok", LastTransitionTime: "old"},
		{Type: core.CondDegraded, Status: "False", Reason: "Healthy", Message: "-", LastTransitionTime: "old"},
	}
	desired := map[string]core.Condition{
		core.CondReady:       {Type: core.CondReady, Status: "True", Reason: "Reconciled", Message: "ok", LastTransitionTime: "new"},
		core.CondProgressing: {Type: core.CondProgressing, Status: "False", Reason: "Idle", Message: "-", LastTransitionTime: "new"},
		core.CondDegraded:    {Type: core.CondDegraded, Status: "False", Reason: "Healthy", Message: "-", LastTransitionTime: "new"},
	}
	merged := mergeConditions(previous, desired)
	if len(merged) != 3 {
		t.Fatalf("expected three conditions, got %d", len(merged))
	}
	if merged[0].Type != core.CondReady || merged[1].Type != core.CondProgressing || merged[2].Type != core.CondDegraded {
		t.Fatalf("conditions returned out of order: %+v", merged)
	}
	for _, cond := range merged {
		if cond.LastTransitionTime != "old" {
			t.Fatalf("expected last transition to be preserved for %s", cond.Type)
		}
	}
}
